...
export default function AdminPage() {
  ...
  const filtered = submissions.filter((s) => {
    const matchesSearch = !search || [s.agentName, s.propertyAddress, s.brokerageName, s.mlsNumber, s.listingPrice, s.ticket]
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-100 bg-slate-50/50">
                        <th className="text-left py-3 px-4 font-semibold text-slate-500 text-xs uppercase tracking-wider">Ticket</th>
                        ...
                        return (
                          <tr key={s.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                            <td className="py-3 px-4">
                              <span className="font-mono text-xs font-semibold text-navy-600 bg-navy-50 px-2 py-1 rounded-md">{s.ticket || "—"}</span>
                            </td>
                            <td className="py-3 px-4">
              ...
              <div className="flex items-start justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm text-slate-400">
                    {selected.ticket && <span className="font-mono font-semibold text-navy-600 mr-2">{selected.ticket}</span>}
                    Submitted {fmtDate(selected.submittedAt).date} at {fmtDate(selected.submittedAt).time}
                  </p>
                </div>
